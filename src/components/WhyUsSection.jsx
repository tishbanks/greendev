...
const points = [
  {
    title: '♻️ Sobriété numérique',
    description: 'Sites et applis rapides, légers et conçus pour limiter leur consommation.',
    bgColor: '#F3EAC2', // Vert sauge profond
  },
  {
    title: '☁️ Hébergement vert',
    description: 'Infrastructures éco-responsables alimentées par énergie renouvelable.',
    bgColor: '#A8D5BA', // Vert doux et frais
  },
  {
    title: '🔗 APIs sur-mesure',
    description: 'Interfaces qui relient vos services et automatisent les échanges.',
    bgColor: '#F3EAC2', // Jaune pâle moderne
  },
  {
    title: '📊 Interfaces interactives',
    description: 'Tableaux de bord clairs pour suivre vos données en temps réel.',
    bgColor: '#C9D6FF', // Bleu pastel professionnel
  },
  {
    title: '🔍 Visibilité renforcée',
    description: 'Référencement intégré dès la conception pour gagner en audience.',
    bgColor: '#FFDBB5', // Orange doux crème
  },
  {
    title: '⏱️ Accompagnement continu',
    description: 'Support et évolutions régulières pour garantir performance et longévité.',
    bgColor: '#B8E0E5', // Bleu-vert apaisant
  },
]
...
export default function WhyUsSection() {
  ...
  useEffect(() => {
    document.title = "Pourquoi choisir GreenDev ? - Services éthiques & performants"
    ...
    meta.content = "GreenDev conçoit des services numériques responsables : sobriété, performance, accessibilité et accompagnement sur mesure."
  }, [])
  ...
  // Index actif basé sur le scroll
  useEffect(() => {
    const handleScroll = () => {
      if (!sectionRef.current) return
      const sectionTop = sectionRef.current.offsetTop
  ...
  // Visibilité de la section via scrollY
  useEffect(() => {
...
{/* Timeline horizontale (mobile centrée) */}
<motion.div
...
>
  {points.map((point, i) => (
    ...
    >
      ...
      {/* Points de référence pour scroll */}
      {points.map((_, i) => (
        <div key={i} ref={pointRefs.current[i]} className="h-screen" />
      ))}
{/* Bloc SEO invisible pour les moteurs de recherche */}
<div style={{ position: 'absolute', left: '-9999px', top: 'auto', width: '1px', height: '1px', overflow: 'hidden' }}>
  <section>
    <h2>Pourquoi choisir Green Dev ?</h2>
    <ul>
      {points.map((point, i) => (
        <li key={i}>
          <article>
    ...
    <p>
      Green Dev conçoit des interfaces durables, performantes et éthiques. Nos sites sont pensés pour un web plus
      sobre, plus rapide et plus utile.
    </p>
  </section>
</div>
